...
struct PagedBrowseResult {
    ...
    bool cancelled = false; // isCancelled fired between pages
};
...
// isCancelled is polled before each page; a cancelled result carries
// the pages fetched so far with `cancelled` set.
