...
| M4 Browser 體驗與 metadata | ✅ 完成 — 全部驗收通過（docs/21），v0.2.0 |
...
### M4 成果（component 版本 `0.2.0`）

全部經真機驗收（逐項證據見 `docs/21_manual_test_checklist.md`）：

- 「DMS Browser」layout element（`ui_element_mac`）整合，可放入自訂 layout
- browser 底部 metadata / resource 摘要與 album art 預覽
- container「加入直接子項曲目」（含未展開 container 隨選取回）與
  「遞迴加入所有曲目」（即時進度、取消、10,000 上限、略過計數）
- playlist / Now Playing 封面（`album_art_fallback` 下載）
- 真實 server 播放與 seek（foo_upnp 0.99.49、MiniDLNA 1.3.3）
- 錯誤處理：手動重試（console 記錄）、SOAP fault 顯示、
  30s 傳輸 timeout（連線 10s）
...
- **M3（✅ 完成）**：CMake macOS bundle + foobar2000 macOS SDK、
  ...
  `docs/adr/ADR-013`）
- **M4（✅ 完成）**：browser metadata/album art、遞迴加入、layout
  element、錯誤處理強化，驗收全數通過（見上方「M4 成果」與
  `docs/21_manual_test_checklist.md`）
- **M5（🔄 進行中）**：server 相容性——MiniDLNA 1.3.3 已完成基線
  （CLI + 元件級，`docs/10`）；Jellyfin / Plex 待測
- **M6**：SSDP discovery、release packaging（`docs/12`；v0.2.0 beta
  打包已就緒，見 `docs/22`）
