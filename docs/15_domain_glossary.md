...
## 附錄：foobar2000 SDK 術語（M3 起使用）

### cfg_var
fb2k SDK 的設定變數模板（`cfg_uint`、`cfg_string`…），值存進 fb2k
設定系統、隨 profile 備份。本專案用單一 `cfg_var<string>` 存 server
清單 JSON（ADR-015）。

### component bundle
mac 版 fb2k 的 component 形式：`foo_*.component` bundle，放在
`~/Library/foobar2000-v2/user-components/`（扁平掃描）。入口符號
`foobar2000_get_interface` 由 `foobar2000_component_client` 提供。

### initquit
fb2k SDK service：component 載入完成（`on_init`）與關閉（`on_quit`）
的 hook。本專案的載入驗證 marker 即由此寫出。

### main_thread_callback
fb2k SDK 機制：把工作排回 fb2k 主執行緒執行。Component 的背景
browse 完成後用它（或 GCD main queue）更新 UI（ADR-017）。

### metadb hint（metadb_hint_list）
fb2k SDK 機制：在不讀取音訊檔的情況下，把已知的 metadata「提示」
給 metadb，playlist 立即顯示。本專案用它預填 DIDL 資料（ADR-016）。

### preferences_page（mac）
fb2k 偏好設定頁的 service。mac 上 `instantiate()` 回傳包裝的
NSViewController（`fb2k::wrapNSObject`）。本專案 programmatic 建構、
不用 xib（ADR-014）。

### ui_element_mac
mac 版 fb2k 的 layout element 介面：`instantiate()` 回傳包裝的
NSViewController，使用者可在 layout 中以名稱引用。SDK 有介面但
無文件無範例；本專案 M4 才驗證（ADR-014）。
