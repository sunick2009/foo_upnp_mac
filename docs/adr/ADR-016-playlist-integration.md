...
2. **Container 右鍵同時保留兩種加入語意。**
   - 「加入直接子項曲目」對已載入的 container 直接使用 tree 中的子項；
     **未載入的 container 則隨選抓取其直接子項再加入**（2026-07-18
     修訂，issue #11——原「只對已載入子項生效」的限制在實測中造成
     「必須先展開才能加入」的困惑）。隨選抓取走同一個 worker queue、
     可被「取消加入」中止（分頁之間輪詢），超過 10,000 項時於狀態列
     標示截斷；加入/略過計數與已載入路徑一致。
